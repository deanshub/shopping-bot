...
export async function loadShoppingLists() {
  const files = await readdir(SHOPPING_LISTS_DIR);
  for (const file of files) {
    const shoppingList = await readFile(
      `${SHOPPING_LISTS_DIR}/${file}`,
      "utf-8"
    );
...
export async function addItemsToShoppingList(items: string[], listId: number) {
  ...
  const itemsToRemove = items.filter(
    (item) => item.includes("לא צריך") || item.includes("קניתי")
  );
  const newItems = items.filter(
    (item) =>
      !originalShoppingList.includes(item) && !itemsToRemove.includes(item)
  );
  ...
  const shoppingList = originalShoppingList.filter((item) => {
    ...
    return !renamedItemsToRemove.some((removeItem) =>
      item.includes(removeItem)
    );
  });
  ...
  await writeFile(
    `${SHOPPING_LISTS_DIR}/${listId}.json`,
    JSON.stringify(newShoppingList, null, 2)
  );
}
